...
type
  TFormEditor = class(TForm)
    ...
    procedure SGCuratorKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
 t : string;
begin
 result := ''; if length(Trim(s)) = 0 then exit; s := Trim(s);
 t := s[1]; delete(s,1,1); t := AnsiUpperCase(t); result := t + s;
end;
...
begin
 if TsSpeedButton(Sender).Name = 'BtnAddCuratorToList' then Adding(SGCurator,EditCurator);
 if TsSpeedButton(Sender).Name = 'BtnAddRubrToList' then Adding(SGRubr,EditRubr);
 if TsSpeedButton(Sender).Name = 'BtnAddTypeToList' then Adding(SGType,EditType);
 if TsSpeedButton(Sender).Name = 'BtnAddNaprToList' then Adding(SGNapravlenie,EditNapravlenie);
end;
...
begin
 sg := nil;
 if TsSpeedButton(Sender).Name = 'BtnDeleteCuratorFromList' then sg := SGCurator;
 if TsSpeedButton(Sender).Name = 'BtnDeleteRubrFromList' then sg := SGRubr;
 if TsSpeedButton(Sender).Name = 'BtnDeleteTypeFromList' then sg := SGType;
 if TsSpeedButton(Sender).Name = 'BtnDeleteNaprFromList' then sg := SGNapravlenie;
 if TsSpeedButton(Sender).Name = 'BtnDeleteWebFromList' then sg := SGWeb;
 if TsSpeedButton(Sender).Name = 'BtnDeleteEMailFromList' then sg := SGEMail;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList1' then sg := SGPhone1;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList2' then sg := SGPhone2;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList3' then sg := SGPhone3;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList4' then sg := SGPhone4;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList5' then sg := SGPhone5;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList6' then sg := SGPhone6;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList7' then sg := SGPhone7;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList8' then sg := SGPhone8;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList9' then sg := SGPhone9;
 if TsSpeedButton(Sender).Name = 'BtnDeletePhoneFromList10' then sg := SGPhone10;
 if Assigned(sg) then begin
  if sg.SelectedCount = 0 then exit;
  sg.DeleteRow(sg.SelectedRow);
  if sg.RowCount > 0 then sg.SelectFirstRow;
 end;
end;
...
begin
 edit := nil; sg := nil;
 if TsSpeedButton(Sender).Name = 'BtnAddWebToList' then begin
  edit := EditWEB; sg := SGWeb;
 end;
 if TsSpeedButton(Sender).Name = 'BtnAddEMailToList' then begin
  edit := EditEMAIL; sg := SGEMail;
 end;
 if Trim(edit.Text) = '' then exit;
 for i := 0 to sg.RowCount - 1 do
 if AnsiLowerCase(sg.Cells[0,i]) = AnsiLowerCase(Trim(edit.Text)) then
 ...
 if TsSpeedButton(Sender).Name = 'BtnAddWebToList' then
  if NOT FormMailSender.IsValidWeb(edit.Text) then begin
//  if (Pos('www.',AnsiLowerCase(edit.Text)) = 0) OR (Pos(',',edit.Text) > 0) OR (Pos(' ',edit.Text) > 0) then begin
 ...
 if TsSpeedButton(Sender).Name = 'BtnAddEMailToList' then
  if NOT FormMailSender.IsValidEmail(edit.Text) then begin
...
//  EditNapravlenie.DroppedDown := True; {Был баг: стерало всю строку едита при выпадании списка}
 if Key = #13 then begin
  Key := #0;
  if TsComboBox(Sender).Name = 'EditCurator' then BtnAddCuratorToListClick(BtnAddCuratorToList);
  if TsComboBox(Sender).Name = 'EditRubr' then BtnAddCuratorToListClick(BtnAddRubrToList);
  if TsComboBox(Sender).Name = 'EditType' then BtnAddCuratorToListClick(BtnAddTypeToList);
  if TsComboBox(Sender).Name = 'EditNapravlenie' then begin
   BtnAddCuratorToListClick(BtnAddNaprToList); // EditNapravlenie.DroppedDown := False;
  end;
  if TsComboBox(Sender).Name = 'EditWEB' then BtnAddWebToListClick(BtnAddWebToList);
  if TsComboBox(Sender).Name = 'EditEMAIL' then BtnAddWebToListClick(BtnAddEMailToList);
  if (TsComboBox(Sender).Name = 'EditPhoneType1') or (TsEdit(Sender).Name = 'EditPhone1')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList1);
  if (TsComboBox(Sender).Name = 'EditPhoneType2') or (TsEdit(Sender).Name = 'EditPhone2')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList2);
  if (TsComboBox(Sender).Name = 'EditPhoneType3') or (TsEdit(Sender).Name = 'EditPhone3')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList3);
  if (TsComboBox(Sender).Name = 'EditPhoneType4') or (TsEdit(Sender).Name = 'EditPhone4')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList4);
  if (TsComboBox(Sender).Name = 'EditPhoneType5') or (TsEdit(Sender).Name = 'EditPhone5')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList5);
  if (TsComboBox(Sender).Name = 'EditPhoneType6') or (TsEdit(Sender).Name = 'EditPhone6')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList6);
  if (TsComboBox(Sender).Name = 'EditPhoneType7') or (TsEdit(Sender).Name = 'EditPhone7')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList7);
  if (TsComboBox(Sender).Name = 'EditPhoneType8') or (TsEdit(Sender).Name = 'EditPhone8')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList8);
  if (TsComboBox(Sender).Name = 'EditPhoneType9') or (TsEdit(Sender).Name = 'EditPhone9')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList9);
  if (TsComboBox(Sender).Name = 'EditPhoneType10') or (TsEdit(Sender).Name = 'EditPhone10')
   then BtnAddPhoneToList1Click(BtnAddPhoneToList10);
 end;
end;

procedure TFormEditor.EditCuratorExit(Sender: TObject);
begin
 if TsComboBox(Sender).Name = 'EditCurator' then EditCurator.Text := ''
 else if TsComboBox(Sender).Name = 'EditRubr' then EditRubr.Text := ''
 else if TsComboBox(Sender).Name = 'EditType' then EditType.Text := ''
 else if TsComboBox(Sender).Name = 'EditNapravlenie' then EditNapravlenie.Text := ''
 else if TsComboBox(Sender).Name = 'EditWEB' then EditWEB.Text := ''
 else if TsComboBox(Sender).Name = 'EditEMAIL' then EditEMAIL.Text := '';
end;

procedure TFormEditor.SGCuratorKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if Key = 46 then begin
  Key := 0;
  if TNextGrid(Sender).Name = 'SGCurator' then BtnDeleteCuratorFromListClick(BtnDeleteCuratorFromList);
  if TNextGrid(Sender).Name = 'SGRubr' then BtnDeleteCuratorFromListClick(BtnDeleteRubrFromList);
  if TNextGrid(Sender).Name = 'SGType' then BtnDeleteCuratorFromListClick(BtnDeleteTypeFromList);
  if TNextGrid(Sender).Name = 'SGNapravlenie' then BtnDeleteCuratorFromListClick(BtnDeleteNaprFromList);
  if TNextGrid(Sender).Name = 'SGWeb' then BtnDeleteCuratorFromListClick(BtnDeleteWebFromList);
  if TNextGrid(Sender).Name = 'SGEMail' then BtnDeleteCuratorFromListClick(BtnDeleteEMailFromList);
  if TNextGrid(Sender).Name = 'SGPhone1' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList1);
  if TNextGrid(Sender).Name = 'SGPhone2' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList2);
  if TNextGrid(Sender).Name = 'SGPhone3' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList3);
  if TNextGrid(Sender).Name = 'SGPhone4' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList4);
  if TNextGrid(Sender).Name = 'SGPhone5' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList5);
  if TNextGrid(Sender).Name = 'SGPhone6' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList6);
  if TNextGrid(Sender).Name = 'SGPhone7' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList7);
  if TNextGrid(Sender).Name = 'SGPhone8' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList8);
  if TNextGrid(Sender).Name = 'SGPhone9' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList9);
  if TNextGrid(Sender).Name = 'SGPhone10' then BtnDeleteCuratorFromListClick(BtnDeletePhoneFromList10);
 end;
end;
...
procedure TFormEditor.SGCuratorDblClick(Sender: TObject);
 ...
 begin
  ...
  if Assigned(edit) then begin
   edit.Text := sg.Cells[0,sg.SelectedRow]; edit.SetFocus;
  end;
  sg.DeleteRow(sg.SelectedRow);
 end;

begin
 if TNextGrid(Sender).Name = 'SGCurator' then Editing(SGCurator,EditCurator);
 if TNextGrid(Sender).Name = 'SGRubr' then Editing(SGRubr,EditRubr);
 if TNextGrid(Sender).Name = 'SGType' then Editing(SGType,EditType);
 if TNextGrid(Sender).Name = 'SGNapravlenie' then Editing(SGNapravlenie,EditNapravlenie);
 if TNextGrid(Sender).Name = 'SGWeb' then Editing(SGWeb,EditWEB);
 if TNextGrid(Sender).Name = 'SGEMail' then Editing(SGEMail,EditEMAIL);
end;

procedure TFormEditor.SGCuratorEnter(Sender: TObject);
begin
 if TNextGrid(Sender).RowCount > 0 then
  if TNextGrid(Sender).SelectedCount = 0 then TNextGrid(Sender).SelectFirstRow;
end;
...
procedure TFormEditor.BtnAddPhoneToList1Click(Sender: TObject);
 ...
 begin
  ...
  if Trim(editPhoneType.Text) = '' then begin
   MessageBox(handle,'Не указан тип телефона','Предупрежение',MB_OK or MB_ICONWARNING); exit;
  end;
  if Trim(editPhone.Text) = '' then begin
   MessageBox(handle,'Не указан телефон','Предупрежение',MB_OK or MB_ICONWARNING); exit;
  end;
  if sg.FindText(0,'('+tp+') '+ph,[soCaseInsensitive,soExactMatch]) then begin
   MessageBox(handle,'Указанный телефон уже есть в списке','Предупрежение',MB_OK or MB_ICONWARNING); exit;
  end;
  sg.AddRow;
  sg.Cells[0,sg.LastAddedRow] := '(' + tp + ') ' + ph;
  editPhoneType.Text := ''; editPhone.Text := ''; editPhoneType.SetFocus;
 end;
...
procedure TFormEditor.btnDeleteAdresClick(Sender: TObject);
 ...
 procedure ClearingEdits(OfficeType,Country,City: TsComboBox;
  ZIP,Street : TsEdit; SGPhone : TNextGrid);
 begin
  OfficeType.Text := ''; Country.Text := ''; City.Text := '';
  ZIP.Text := ''; Street.Text := ''; SGPhone.ClearRows;
 end;
...
begin
 ...
 FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
 Application.ProcessMessages;
 for z := 1 to 10 do TsComboBox(FindComponent('EditOfficeType'+IntToStr(z))).Clear;
 ...
 begin
  for z := 1 to 10 do TsComboBox(FindComponent('EditOfficeType'+IntToStr(z))).AddItem
   (Q.FieldValues['NAME'],Pointer(Integer(Q.FieldValues['ID'])));
  Q.Next;
 end;
 FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
 Application.ProcessMessages;
 for z := 1 to 10 do TsComboBox(FindComponent('EditCountry'+IntToStr(z))).Clear;
 Q.Close; // СТРАНЫ
 Q.SQL.Text := 'select * from COUNTRY order by lower(NAME)';
 Q.Open; Q.FetchAll;
 for i := 1 to Q.RecordCount do
 begin
  for z := 1 to 10 do TsComboBox(FindComponent('EditCountry'+IntToStr(z))).AddItem
   (Q.FieldValues['NAME'],Pointer(Integer(Q.FieldValues['ID'])));
  Q.Next;
 end;
 FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
 Application.ProcessMessages;
 for z := 1 to 10 do TsComboBox(FindComponent('EditCity'+IntToStr(z))).Clear;
 Q.Close; // ГОРОДА
 Q.SQL.Text := 'select * from GOROD order by lower(NAME)';
 Q.Open; Q.FetchAll;
 for i := 1 to Q.RecordCount do
 begin
  for z := 1 to 10 do TsComboBox(FindComponent('EditCity'+IntToStr(z))).AddItem
   (Q.FieldValues['NAME'],Pointer(Integer(Q.FieldValues['ID'])));
  Q.Next;
 end;
 FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
 Application.ProcessMessages;
 for z := 1 to 10 do TsComboBox(FindComponent('EditPhoneType'+IntToStr(z))).Clear;
 Q.Close; // ТИПЫ ТЕЛЕФОНОВ
 Q.SQL.Text := 'select * from PHONETYPE order by lower(NAME)';
 Q.Open; Q.FetchAll;
 for i := 1 to Q.RecordCount do
 begin
  for z := 1 to 10 do TsComboBox(FindComponent('EditPhoneType'+IntToStr(z))).AddItem
   (Q.FieldValues['NAME'],Pointer(Integer(Q.FieldValues['ID'])));
  Q.Next;
 end;
 FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
...
begin
 ...
 FormMain.IBQuery1.Close; FormMain.IBDatabase1.Close;
end;
...
begin
 result := '';
 if (Trim(table) = '') or (Trim(id) = '') then exit;
 Q := TIBQuery.Create(FormMain);
 Q.Database := FormMain.IBDatabase1;
 Q.Transaction := FormMain.IBTransaction1;
 Q.Close; Q.Sql.Text := 'select * from ' + table + ' where id = ' + id;
 Q.Open; Q.FetchAll;
 if Q.RecordCount > 0 then result := Q.FieldValues['NAME'];
 Q.Close; Q.Free;
end;
...
begin
 Result := '';
 if component.RowCount = 0 then exit;
 ...
 begin
  ...
  if component.Name = 'SGWeb' then tmp := tmp + SGWeb.Cells[0,i] + ', ';
  if component.Name = 'SGEMail' then tmp := tmp + SGEMail.Cells[0,i] + ', ';
 end;
 Result := tmp;
end;
...
begin
 ...
 tmp := Trim(tmp);
 if Length(tmp) > 0 then if tmp[Length(tmp)] = ',' then delete(tmp,Length(tmp),1);
 Result := tmp;
end;
...
var
 i : integer;
begin
  lblID.Caption := ''; SGCurator.ClearRows; SGRubr.ClearRows; SGType.ClearRows;
  SGNapravlenie.ClearRows; SGWeb.ClearRows; SGEMail.ClearRows;
  CBActivity.Checked := True; cbDataRelevance.Checked := True;
  sPageControl1.ActivePageIndex := 0;
  EditName.Text := ''; EditFIO.Text := ''; EditCurator.Text := '';
  EditRubr.Text := ''; EditType.Text := '';
  EditNapravlenie.Text := ''; EditWEB.Text := ''; EditEMAIL.Text := '';
...
var
 ...
 procedure AdresProcs(CBAdres:TsCheckBox; No:TsEdit; OfficeType:TsComboBox;
  Zip:TsEdit; Street:TsEdit; Country:TsComboBox; City:TsComboBox; MemoPhone:TsMemo; SGPhone:TNextGrid);
 var
  i : integer;
 ...
 begin
  ...
  begin
   ...
   str.Add('#1$#'+NO.Text+'$#@'+offtype_id+'$#'+Trim(ZIP.Text)+'$#'+
   Trim(Street.Text)+'$#&'+country_id+'$#^'+city_id+'$');
   if SGPhone.RowCount > 0 then for i := 0 to SGPhone.RowCount - 1 do
 ...
 procedure IsAdresFilled(CBAdres:TsCheckBox; OfficeType:TsComboBox; Zip:TsEdit;
  Street:TsEdit; Country:TsComboBox; City:TsComboBox; SGPhone:TNextGrid);
 begin
  CBAdres.Checked := False;
  if Trim(OfficeType.Text) <> '' then begin CBAdres.Checked := True; exit; end;
  if Trim(Zip.Text) <> '' then begin CBAdres.Checked := True; exit; end;
  if Trim(Street.Text) <> '' then begin CBAdres.Checked := True; exit; end;
  if Trim(Country.Text) <> '' then begin CBAdres.Checked := True; exit; end;
  if Trim(City.Text) <> '' then begin CBAdres.Checked := True; exit; end;
  if SGPhone.RowCount > 0 then begin CBAdres.Checked := True; exit; end;
 end;
 ...
 begin
  Result := '';
  if (SGRubr.RowCount = 0) OR (SGNapravlenie.RowCount = 0) then exit;
  for x := 0 to SGRubr.RowCount - 1 do begin
   idRUBR := SGRubr.Cells[1,x];
   finalSTR := finalSTr + '#' + idRUBR + '=';
   for z := 0 to SGNapravlenie.RowCount - 1 do begin
...
begin
 ...
 if NOT(IsDublicate) then begin
  FormEditor.Close; IsRecordDublicate; exit;
 ...
 if FormDublicate.Visible then FormDublicate.Close;
 IsNewRecordCheck;
 ...
 str := TStringList.Create; phones := TStringList.Create;
 ...
 begin
  ...
  if nd <> nil then begin
   nd2 := FormMain.TVRubrikator.Items.AddChildObject(nd ,
    EditName.Text, Pointer(StrToInt(ID)));
   if CBActivity.Checked then begin
    nd2.ImageIndex := 1; nd2.SelectedIndex := 1;
   end else begin
    nd2.ImageIndex := 2; nd2.SelectedIndex := 2;
   end;
  end;
 end;
...
procedure TFormEditor.PrepareEditRecord(ID : string);
 ...
 procedure AdresProcs(AdresList:TStrings; Num:Integer; CBAdres:TsCheckBox;
  OfficeType:TsComboBox; Zip:TsEdit; Street:TsEdit; Country:TsComboBox; City:TsComboBox);
 var
  tmp,city_str,country_str,ofType,s : string;
  list : TStrings;
 begin
  if Length(AdresList.Text) = 0 then exit; // Грузятся адреса {begin}
  tmp := AdresList[Num - 1];
  ...
  if list[0] = '0' then
  begin
   CBAdres.Checked := False; list.Free; exit;
  end;
  // такая же процедура в Main.OpenTabByID и Report.GenerateReport и ReportSimple.GenerateReport
  if list[0] = '1' then CBAdres.Checked := True;
  city_str := list[6]; if city_str[1] = '^' then delete(city_str,1,1);
  country_str := list[5]; if country_str[1] = '&' then delete(country_str,1,1);
  ofType := list[2]; if ofType[1] = '@' then delete(ofType,1,1);
 ...
 procedure PhonesProcs(PhonesList: TStrings);
  ...
  begin
   ...
   for i := 0 to MemoPhone.Lines.Count - 1 do begin
    SGPhone.AddRow; SGPhone.Cells[0,SGPhone.LastAddedRow] := MemoPhone.Lines[i];
   end; MemoPhone.Clear;
  end;
 ...
 begin
  ...
  begin
   ...
   delete(ph_tmp,1,1); delete(ph_tmp,length(ph_tmp),1);
   ph_tmp2 := copy(ph_list ,0 , pos('$',ph_list));
   delete(ph_list , 1 , length(ph_tmp2));
   delete(ph_tmp2,1,1); delete(ph_tmp2,length(ph_tmp2),1);
   if (ph_tmp = '1') AND (CBAdres1.Checked) then Loading(MemoPhone1,SGPhone1,ph_tmp2);
   if (ph_tmp = '2') AND (CBAdres2.Checked) then Loading(MemoPhone2,SGPhone2,ph_tmp2);
   if (ph_tmp = '3') AND (CBAdres3.Checked) then Loading(MemoPhone3,SGPhone3,ph_tmp2);
   if (ph_tmp = '4') AND (CBAdres4.Checked) then Loading(MemoPhone4,SGPhone4,ph_tmp2);
   if (ph_tmp = '5') AND (CBAdres5.Checked) then Loading(MemoPhone5,SGPhone5,ph_tmp2);
   if (ph_tmp = '6') AND (CBAdres6.Checked) then Loading(MemoPhone6,SGPhone6,ph_tmp2);
   if (ph_tmp = '7') AND (CBAdres7.Checked) then Loading(MemoPhone7,SGPhone7,ph_tmp2);
   if (ph_tmp = '8') AND (CBAdres8.Checked) then Loading(MemoPhone8,SGPhone8,ph_tmp2);
   if (ph_tmp = '9') AND (CBAdres9.Checked) then Loading(MemoPhone9,SGPhone9,ph_tmp2);
   if (ph_tmp = '10') AND (CBAdres10.Checked) then Loading(MemoPhone10,SGPhone10,ph_tmp2);
  end;
 end;
 ...
 var
  tmp,name,id : string;
 begin
  if Length(Trim(str)) = 0 then exit;
  ...
  begin
   ...
   if component.Name = 'SGCurator' then
    if Main.sgCurator_tmp.FindText(1,tmp,[soCaseInsensitive,soExactMatch]) then
     begin name := Main.sgCurator_tmp.Cells[0,Main.sgCurator_tmp.SelectedRow];
           id := Main.sgCurator_tmp.Cells[1,Main.sgCurator_tmp.SelectedRow]; end;
   if component.Name = 'SGRubr' then
    if Main.sgRubr_tmp.FindText(1,tmp,[soCaseInsensitive,soExactMatch]) then
     begin name := Main.sgRubr_tmp.Cells[0,Main.sgRubr_tmp.SelectedRow];
           id := Main.sgRubr_tmp.Cells[1,Main.sgRubr_tmp.SelectedRow]; end;
   if component.Name = 'SGType' then
    if Main.sgType_tmp.FindText(1,tmp,[soCaseInsensitive,soExactMatch]) then
     begin name := Main.sgType_tmp.Cells[0,Main.sgType_tmp.SelectedRow];
           id := Main.sgType_tmp.Cells[1,Main.sgType_tmp.SelectedRow]; end;
   if component.Name = 'SGNapravlenie' then
    if Main.sgNapr_tmp.FindText(1,tmp,[soCaseInsensitive,soExactMatch]) then
     begin name := Main.sgNapr_tmp.Cells[0,Main.sgNapr_tmp.SelectedRow];
           id := Main.sgNapr_tmp.Cells[1,Main.sgNapr_tmp.SelectedRow]; end;
    if (Trim(name) <> '') AND (Trim(id) <> '') then begin
     component.AddRow;
 ...
 begin
  ...
  begin
   tmp := copy(str ,0 , pos(',',str));
   delete(str, 1 , length(tmp));
   delete(tmp,length(tmp),1); tmp := Trim(tmp);
   if tmp <> '' then begin
    component.AddRow;
...
begin
 ...
 for i := FormMain.TVRubrikator.Items.Count - 1 downto 0 do
  if (IntToStr(Integer(Pointer(FormMain.TVRubrikator.Items[i].Data))) = lblID.Caption)
     AND (FormMain.TVRubrikator.Items[i].Level <> 0)
       then FormMain.TVRubrikator.Items[i].Delete;
 rubrID := NewRubr;
 ...
 begin
  ...
  if nd <> nil then begin
   nd2 := FormMain.TVRubrikator.Items.AddChildObject(nd ,
    EditName.Text, Pointer(StrToInt(lblID.Caption)));
   if CBActivity.Checked then begin
    nd2.ImageIndex := 1;
    nd2.SelectedIndex := 1;
   end else begin
...
begin
 ...
 Q.Database := FormMain.IBDatabase1;
 Q.Transaction := FormMain.IBTransaction1;
 Q.Close; Q.SQL.Text := 'select NAME from BASE where ID = :ID';
 Q.ParamByName('ID').AsString := ID;
 Q.Open;
 capt := Q.Fields[0].Value;
 if MessageBox(handle,'Запись будет удалена. Продолжить?',PAnsiChar(Capt),MB_YESNO or MB_ICONQUESTION) = MRNO
  then begin Q.Close; Q.Free; exit; end;
...
// FormMain.IBDatabase1.Close;
 for i := FormMain.TVRubrikator.Items.Count - 1 downto 0 do
  if (IntToStr(Integer(Pointer(FormMain.TVRubrikator.Items[i].Data))) = ID)
     AND (FormMain.TVRubrikator.Items[i].Level <> 0)
       then FormMain.TVRubrikator.Items[i].Delete;
 for i := FormMain.SGGeneral.RowCount - 1 downto 0 do
  if FormMain.SGGeneral.Cells[0,i] = ID then
   begin FormMain.SGGeneral.DeleteRow(i); break; end;
...
var
 ...
 begin
  ...
  id := Q.Fields[0].Value;
  Q.Close; Q.Free;
  if AnsiLowerCase(table) = 'curator' then begin
   SGCurator.Cells[1,i] := id;
   EditCurator.AddItem(value,Pointer(StrToInt(id)));
  end;
  if AnsiLowerCase(table) = 'rubrikator' then begin
...
begin
 ...
 begin
  AddingNewRec('CURATOR',SGCurator.Cells[0,i],
   Main.sgCurator_tmp,FormDirectory.SGCurator);
 end;
 for i := 0 to SGRubr.RowCount - 1 do
 if SGRubr.Cells[1,i] = '' then
 begin
  AddingNewRec('RUBRIKATOR',SGRubr.Cells[0,i],
   Main.sgRubr_tmp,FormDirectory.SGRubr);
  isNewRubr := True;
 end;
 for i := 0 to SGType.RowCount - 1 do
 if SGType.Cells[1,i] = '' then
 begin
  AddingNewRec('TYPE',SGType.Cells[0,i],
   Main.sgType_tmp,FormDirectory.SGFirmType);
 end;
 for i := 0 to SGNapravlenie.RowCount - 1 do
 if SGNapravlenie.Cells[1,i] = '' then
 begin
  AddingNewRec('NAPRAVLENIE',SGNapravlenie.Cells[0,i],
   Main.sgNapr_tmp,FormDirectory.SGNapr);
 end;
 ...
 if isNewRubr then FormMain.TVRubrikator.CustomSort(@CustomSortProc, 0,True);
end;
...
var
 ...
 var
  z : integer;
 begin
  if SGPhone.RowCount > 0 then begin
   for z := 0 to SGPhone.RowCount - 1 do begin
    PHONEstr := SGPhone.Cells[0,z];
    delete(PHONEstr,1,Pos(')',PHONEstr)+1);
    PHONEstr := AnsiLowerCase(Trim(PHONEstr));
    if PHONElist.IndexOf(PHONEstr) = -1 then PHONElist.Add(PHONEstr);
   end;
  end;
 end;
...
begin
 ...
 if Length(WEBstr) > 0 then begin
  if WEBstr[Length(WEBstr)] <> ',' then WEBstr := WEBstr + ',';
  while pos(',',WEBstr) > 0 do begin
   WEBtmp := copy(WEBstr ,0 , pos(',',WEBstr));
   delete(WEBstr, 1 , length(WEBtmp));
   delete(WEBtmp,length(WEBtmp),1); WEBtmp := AnsiLowerCase(Trim(WEBtmp));
   WEBlist.Add(WEBtmp);
  end;
 end;
 EMAILstr := GetWebEmailString(SGEMail);
 EMAILlist := TStringList.Create;
 if Length(EMAILstr) > 0 then begin
  if EMAILstr[Length(EMAILstr)] <> ',' then EMAILstr := EMAILstr + ',';
  while pos(',',EMAILstr) > 0 do begin
   EMAILtmp := copy(EMAILstr ,0 , pos(',',EMAILstr));
   delete(EMAILstr, 1 , length(EMAILtmp));
   delete(EMAILtmp,length(EMAILtmp),1); EMAILtmp := AnsiLowerCase(Trim(EMAILtmp));
   EMAILlist.Add(EMAILtmp);
  end;
 end;

 PHONElist := TStringList.Create;
 GetPhoneList(SGPhone1); GetPhoneList(SGPhone2);
 GetPhoneList(SGPhone3); GetPhoneList(SGPhone4);
 GetPhoneList(SGPhone5); GetPhoneList(SGPhone6);
 GetPhoneList(SGPhone7); GetPhoneList(SGPhone8);
 GetPhoneList(SGPhone9); GetPhoneList(SGPhone10);

// REQ1 := ' lower(NAME) = ''' + Name + ''' or';
 REQ1 := ' lower(NAME) = :NAME or';
 if WEBlist.Count > 0 then for x := 0 to WEBlist.Count - 1 do
  REQ2 := REQ2 + ' lower(WEB) like ''%' + WEBlist[x] + '%'' or';
 if EMAILlist.Count > 0 then for x := 0 to EMAILlist.Count - 1 do
  REQ3 := REQ3 + ' lower(EMAIL) like ''%' + EMAILlist[x] + '%'' or';
 if PHONElist.Count > 0 then for x := 0 to PHONElist.Count - 1 do
  REQ4 := REQ4 + ' lower(PHONES) like ''%' + PHONElist[x] + '%'' or';
 REQ := 'select * from BASE where' + REQ1 + REQ2 + REQ3 + REQ4;
 delete(REQ,Length(REQ)-2,3);
 WEBlist.Free; EMAILlist.Free; PHONElist.Free;

 Q := TIBQuery.Create(FormEditor);
 Q.Database := FormMain.IBDatabase1;
 Q.Transaction := FormMain.IBTransaction1;
 Q.Close; Q.SQL.Text := REQ;
 ...
 if Q.RecordCount > 0 then begin
  ...
  for x := 1 to Q.RecordCount do begin
   FormDublicate.SGDublicate.AddRow;
   if Q.FieldValues['ACTIVITY'] = '-1' then
    FormDublicate.SGDublicate.Cells[0,FormDublicate.SGDublicate.LastAddedRow] := '1' else
