...
begin
 if isFirstShow then LoadDataDirectory;
 isFirstShow := False;
 editCurator.Clear; editRubr.Clear; editFirmType.Clear; editNapr.Clear;
 editOfficeType.Clear; editCountry.Clear; editCity.Clear; editPhoneType.Clear;
end;
...
var
 ...
 i : integer;
begin
 SGCurator.BeginUpdate; SGCurator.ClearRows;
 ...
 SGCurator.EndUpdate;
 SGRubr.BeginUpdate; SGRubr.ClearRows;
 ...
 SGRubr.EndUpdate;
 SGFirmType.BeginUpdate; SGFirmType.ClearRows;
 ...
 SGNapr.BeginUpdate; SGNapr.ClearRows;
 ...
 SGOfficeType.BeginUpdate; SGOfficeType.ClearRows;
 if Q_Dir.RecordCount > 0 then for i := 1 to Q_Dir.RecordCount do
 begin
 ...
 Q_Dir.Open; Q_Dir.FetchAll;
 SGCountry.BeginUpdate; SGCountry.ClearRows;
 if Q_Dir.RecordCount > 0 then for i := 1 to Q_Dir.RecordCount do
 begin
 ...
 Q_Dir.Open; Q_Dir.FetchAll;
 SGCity.BeginUpdate; SGCity.ClearRows;
 if Q_Dir.RecordCount > 0 then for i := 1 to Q_Dir.RecordCount do
 begin
 ...
 Q_Dir.Open; Q_Dir.FetchAll;
 SGPhoneType.BeginUpdate; SGPhoneType.ClearRows;
 if Q_Dir.RecordCount > 0 then for i := 1 to Q_Dir.RecordCount do
 begin
...
begin
 ...
 if TsEdit(Sender).Name = 'editCurator' then SG := SGCurator;
 if TsEdit(Sender).Name = 'editRubr' then SG := SGRubr;
 if TsEdit(Sender).Name = 'editFirmType' then SG := SGFirmType;
 if TsEdit(Sender).Name = 'editNapr' then SG := SGNapr;
 if TsEdit(Sender).Name = 'editOfficeType' then SG := SGOfficeType;
 if TsEdit(Sender).Name = 'editCountry' then SG := SGCountry;
 if TsEdit(Sender).Name = 'editCity' then SG := SGCity;
 if TsEdit(Sender).Name = 'editPhoneType' then SG := SGPhoneType;
 SG.BeginUpdate;
...
begin
 ...
 if Trim(s) <> '' then begin
  ...
  case PageControl.ActivePageIndex of
   0 : req := 'select ID from CURATOR where lower(NAME) = :NAME';
   1 : req := 'select ID from RUBRIKATOR where lower(NAME) = :NAME';
   2 : req := 'select ID from TYPE where lower(NAME) = :NAME';
   3 : req := 'select ID from NAPRAVLENIE where lower(NAME) = :NAME';
   4 : req := 'select ID from OFFICETYPE where lower(NAME) = :NAME';
   5 : req := 'select ID from COUNTRY where lower(NAME) = :NAME';
   6 : req := 'select ID from GOROD where lower(NAME) = :NAME';
   7 : req := 'select ID from PHONETYPE where lower(NAME) = :NAME';   
  end;
  Q_Dir := TIBQuery.Create(FormDirectory);
  Q_Dir.Database := FormMain.IBDatabase1;
  Q_Dir.Transaction := FormMain.IBTransaction1;
  ...
  begin
   ...
   Q_Dir.Close; Q_Dir.Free; exit;
  end;
  case PageControl.ActivePageIndex of
   0 : req := 'insert into CURATOR (NAME) values (:NAME)';
   1 : req := 'insert into RUBRIKATOR (NAME) values (:NAME)';
   2 : req := 'insert into TYPE (NAME) values (:NAME)';
   3 : req := 'insert into NAPRAVLENIE (NAME) values (:NAME)';
   4 : req := 'insert into OFFICETYPE (NAME) values (:NAME)';
   5 : req := 'insert into COUNTRY (NAME) values (:NAME)';
   6 : req := 'insert into GOROD (NAME) values (:NAME)';
   7 : req := 'insert into PHONETYPE (NAME) values (:NAME)';   
  end;
  Q_Dir.Close;
  Q_Dir.SQL.Text := req;
  ...
  try Q_Dir.ExecSQL;
   ...
   Q_Dir.Close; Q_Dir.Free; exit; end;
  end;
  FormMain.IBTransaction1.CommitRetaining;
  isDataEdited := True;
  Q_Dir.Close;
  case PageControl.ActivePageIndex of
   0 : Q_Dir.SQL.Text := 'select MAX(ID) from CURATOR';
   1 : Q_Dir.SQL.Text := 'select MAX(ID) from RUBRIKATOR';
   2 : Q_Dir.SQL.Text := 'select MAX(ID) from TYPE';
   3 : Q_Dir.SQL.Text := 'select MAX(ID) from NAPRAVLENIE';
   4 : Q_Dir.SQL.Text := 'select MAX(ID) from OFFICETYPE';
   5 : Q_Dir.SQL.Text := 'select MAX(ID) from COUNTRY';
   6 : Q_Dir.SQL.Text := 'select MAX(ID) from GOROD';
   7 : Q_Dir.SQL.Text := 'select MAX(ID) from PHONETYPE';   
  end;
  Q_Dir.Open;
  ...
  if PageControl.ActivePageIndex = 0 then begin
   ...
   SGCurator.SelectLastRow; SGCurator.SetFocus;
  end else if PageControl.ActivePageIndex = 1 then begin
   SGRubr.AddRow; // РУБРИКИ
   SGRubr.Cells[0,SGRubr.LastAddedRow] := s;
   SGRubr.Cells[1,SGRubr.LastAddedRow] := max_id;
   SGRubr.SelectLastRow; SGRubr.SetFocus;
  end else if PageControl.ActivePageIndex = 2 then begin
   SGFirmType.AddRow; // ТИП ФИРМЫ
   SGFirmType.Cells[0,SGFirmType.LastAddedRow] := s;
   SGFirmType.Cells[1,SGFirmType.LastAddedRow] := max_id;
   SGFirmType.SelectLastRow; SGFirmType.SetFocus;
  end else if PageControl.ActivePageIndex = 3 then begin
   SGNapr.AddRow; // ДЕЯТЕЛЬНОСТЬ
   SGNapr.Cells[0,SGNapr.LastAddedRow] := s;
   SGNapr.Cells[1,SGNapr.LastAddedRow] := max_id;
   SGNapr.SelectLastRow; SGNapr.SetFocus;
  end else if PageControl.ActivePageIndex = 4 then begin
   ...
   SGOfficeType.SelectLastRow; SGOfficeType.SetFocus;
  end else if PageControl.ActivePageIndex = 5 then begin
   SGCountry.AddRow; // СТРАНЫ
   SGCountry.Cells[0,SGCountry.LastAddedRow] := s;
   SGCountry.Cells[1,SGCountry.LastAddedRow] := max_id;
   SGCountry.SelectLastRow; SGCountry.SetFocus;
  end else if PageControl.ActivePageIndex = 6 then begin
   SGCity.AddRow; // ГОРОДА
   SGCity.Cells[0,SGCity.LastAddedRow] := s;
   SGCity.Cells[1,SGCity.LastAddedRow] := max_id;
   SGCity.SelectLastRow; SGCity.SetFocus;
  end else if PageControl.ActivePageIndex = 7 then begin
   SGPhoneType.AddRow; // ТИПЫ ТЕЛЕФОНОВ
   SGPhoneType.Cells[0,SGPhoneType.LastAddedRow] := s;
   SGPhoneType.Cells[1,SGPhoneType.LastAddedRow] := max_id;
   SGPhoneType.SelectLastRow; SGPhoneType.SetFocus;
  end;
  Q_Dir.Close; Q_Dir.Free;
 end;
end;
...
begin
 ...
 if Trim(s) <> '' then begin
  ...
  case PageControl.ActivePageIndex of
   0 : req := 'update CURATOR set NAME = :NAME where ID = :ID';
   1 : req := 'update RUBRIKATOR set NAME = :NAME where ID = :ID';
   2 : req := 'update TYPE set NAME = :NAME where ID = :ID';
   3 : req := 'update NAPRAVLENIE set NAME = :NAME where ID = :ID';
   4 : req := 'update OFFICETYPE set NAME = :NAME where ID = :ID';
   5 : req := 'update COUNTRY set NAME = :NAME where ID = :ID';
   6 : req := 'update GOROD set NAME = :NAME where ID = :ID';
   7 : req := 'update PHONETYPE set NAME = :NAME where ID = :ID';   
  end;
  Q_Dir.Close;
  Q_Dir.SQL.Text := req;
  s := FormEditor.UpperFirst(s);
  ...
  FormMain.IBTransaction1.CommitRetaining;
  isDataEdited := True;
  if Assigned(grid) then grid.Cells[0,grid.SelectedRow] := s;
  Q_Dir.Close; Q_Dir.Free;
 end;
end;
...
procedure TFormDirectory.btnDeleteClick(Sender: TObject);
 ...
 begin
  ...
  if (Trim(field) = '') or (Trim(match) = '') then exit;
  ...
  if Q.RecordCount > 0 then begin
   count := Q.RecordCount; result := true;
  end;
...
begin
 ...
 if PageControl.ActivePageIndex = 0 then begin
  ...
  id := SGCurator.Cells[1,SGCurator.SelectedRow];
  grid := SGCurator;
  if CheckInUse('CURATOR','%#'+id+'$%',c) then begin
  ...
  id := SGRubr.Cells[1,SGRubr.SelectedRow];
  grid := SGRubr;
  if CheckInUse('RUBR','%#'+id+'$%',c) then begin
  ...
  id := SGFirmType.Cells[1,SGFirmType.SelectedRow];
  grid := SGFirmType;
  if CheckInUse('TYPE','%#'+id+'$%',c) then begin
  ...
  id := SGNapr.Cells[1,SGNapr.SelectedRow];
  grid := SGNapr;
  if CheckInUse('NAPRAVLENIE','%#'+id+'$%',c) then begin
  ...
  id := SGOfficeType.Cells[1,SGOfficeType.SelectedRow];
  grid := SGOfficeType;
  if CheckInUse('ADRES','%#@'+id+'$%',c) then begin
 ...
 if MessageBox(handle,'Вы уверены что хотите удалить выбранный объект?','Удалить',
  MB_YESNO or MB_ICONQUESTION) = MRYES then
 begin
  case PageControl.ActivePageIndex of
   0 : req := 'delete from CURATOR where ID = :ID';
   1 : req := 'delete from RUBRIKATOR where ID = :ID';
   2 : req := 'delete from TYPE where ID = :ID';
   3 : req := 'delete from NAPRAVLENIE where ID = :ID';
   4 : req := 'delete from OFFICETYPE where ID = :ID';
   5 : req := 'delete from COUNTRY where ID = :ID';
   6 : req := 'delete from GOROD where ID = :ID';
   7 : req := 'delete from PHONETYPE where ID = :ID';
  end;
  Q_Dir := TIBQuery.Create(FormDirectory);
  Q_Dir.Database := FormMain.IBDatabase1;
  Q_Dir.Transaction := FormMain.IBTransaction1;
  ...
  FormMain.IBTransaction1.CommitRetaining;
  isDataEdited := True;
  if Assigned(grid) then grid.DeleteRow(grid.SelectedRow);
  Q_Dir.Close; Q_Dir.Free;
 end;
end;
