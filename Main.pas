...
type
  TFormMain = class(TForm)
    ...
    procedure sPageControl1MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure TVRubrikatorContextPopup(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
    ...
    procedure SGAddRow(Grid: TNextGrid; Activity,Relevance: integer; Name,
     Curator,DateAdded,DateEdited,Web,Email,fType,ID,FIO,Rubr: string);
    ...
    procedure SGGeneralMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
...
begin
 ...
 if StayActive <> 'formmain' then FormMain.Enabled := False;
 if StayActive <> 'formeditor' then FormEditor.Enabled := False;
 if StayActive <> 'formreport' then FormReport.Enabled := False;
 if StayActive <> 'formmailsender' then FormMailSender.Enabled := False;
 if StayActive <> 'formdirectory' then FormDirectory.Enabled := False;
 if StayActive <> 'formreportsimple' then FormReportSimple.Enabled := False;
 if StayActive <> 'formrelations' then FormRelations.Enabled := False;
 if StayActive <> 'formdublicare' then FormDublicate.Enabled := False;
end;

procedure TFormMain.EnableAllForms(StayNotActive : String);
begin
 StayNotActive := AnsiLowerCase(StayNotActive);
 if StayNotActive <> 'formmain' then FormMain.Enabled := True;
 if StayNotActive <> 'formeditor' then FormEditor.Enabled := True;
 if StayNotActive <> 'formreport' then FormReport.Enabled := True;
 if StayNotActive <> 'formmailsender' then FormMailSender.Enabled := True;
 if StayNotActive <> 'formdirectory' then FormDirectory.Enabled := True;
 if StayNotActive <> 'formreportsimple' then FormReportSimple.Enabled := True;
 if StayNotActive <> 'formrelations' then FormRelations.Enabled := True;
 if StayNotActive <> 'formdublicare' then FormDublicate.Enabled := True;
end;
...
begin
 ...
 else begin RaiseLastOSError; Result := 0 end;
end;
...
procedure TFormMain.sPageControl1MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
 if Button = mbRight then
 begin
  if Y <= 19 then // учим PageControl понимать правый клик только по шапке
  PopupMenu_Tabs.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y) else exit;
 end;
end;

procedure TFormMain.TVRubrikatorContextPopup(Sender: TObject;
...
begin
 ...
 if Assigned(treeNode) then // учим TreeView понимать правый клик
  TVRubrikator.Selected := treeNode else Handled := true;
end;

function TFormMain.SearchNode(component: TsTreeView;
  id: integer; itemlevel: integer): TTreeNode;
var
 Searching : boolean;
 Noddy : TTreeNode;
begin
 Result := nil;
 if component.Items.Count = 0 then exit;
...
begin
 if TForm(Sender).Name <> 'FormEditor' then FormEditor.Close;
 if TForm(Sender).Name <> 'FormReport' then FormReport.Close;
 if TForm(Sender).Name <> 'FormMailSender' then FormMailSender.Close;
 if TForm(Sender).Name <> 'FormDirectory' then FormDirectory.Close;
 if TForm(Sender).Name <> 'FormReportSimple' then FormReportSimple.Close;
 if TForm(Sender).Name <> 'FormRelations' then FormRelations.Close;
 if TForm(Sender).Name <> 'FormDublicate' then FormDublicate.Close;
 DisableAllForms('');
...
begin
 ...
 if NOT Assigned (sgCurator_tmp) then begin
 ...
 end; sgCurator_tmp.ClearRows;
 if NOT Assigned (sgRubr_tmp) then begin
 ...
 end; sgRubr_tmp.ClearRows;
 if NOT Assigned (sgType_tmp) then begin
 ...
 end; sgType_tmp.ClearRows;
 if NOT Assigned (sgNapr_tmp) then begin
 ...
 begin
  ...
  Q_LTT.Next; Application.ProcessMessages;
 end;
 sgRubr_tmp.Resort;
...
// T := now;
 ...
 for i := 1 to IBQuery1.RecordCount  do begin
  if TForm(Sender).Name = 'FormMain' then begin
   FormLogo.sGauge1.Progress := FormLogo.sGauge1.Progress + 1;
  ...
  tmp := TVRubrikator.Items.AddChildObject(nil ,
   IBQuery1.FieldByName('Name').AsString,
    Pointer(IBQuery1.FieldByName('ID').AsInteger));
  tmp.ImageIndex := 0;
  tmp.SelectedIndex := 0;
  ...
  begin
   tmp := TVRubrikator.Items.AddChildObject(Node ,
    Q.FieldByName('Name').AsString ,
     Pointer(Q.FieldByName('ID').AsInteger));
   if Q.FieldByName('ACTIVITY').AsInteger = -1 then begin
    tmp.ImageIndex := 1; tmp.SelectedIndex := 1;
   end else begin
    tmp.ImageIndex := 2; tmp.SelectedIndex := 2;
   end;
   Q.Next;
  end;
  Q.Close; Q.Free;
 ...
 if TVRubrikator.Items.Count > 0  then TVRubrikator.Items[0].Selected := True;
 TVRubrikator.Items.EndUpdate;
 IBDatabase1.Close;
...
begin
 ...
 MailSend.memoLog.Free; MailSend.REFirmInfo.Free; MailSend.editEmailList_tmp.Free;
 // с формы FormMain
...
begin
 if FileExists(AppPath+'office.ini') then exit;
...
begin
 ...
 if sorted_kind = 0 then SGGeneral.Columns[sorted_num].SortKind := skAscending;
 if sorted_kind = 1 then SGGeneral.Columns[sorted_num].SortKind := skDescending;
 IniMain.Free;
end;
...
begin
 ...
 case FormMain.WindowState of
  ...
  wsMaximized : IniMain.WriteInteger('main','wndstate',1);
 end;
 ...
 for i := 2 to 11 do if SGGeneral.Columns[i].Sorted then begin
  if SGGeneral.Columns[i].SortKind = skAscending then IniMain.WriteInteger('sggeneral','col_sorted_kind',0);
  if SGGeneral.Columns[i].SortKind = skDescending then IniMain.WriteInteger('sggeneral','col_sorted_kind',1);
  IniMain.WriteInteger('sggeneral','col_sorted_num',i); break;
 end;
 {**************** FormMailSender *****************}
...
procedure TFormMain.SGAddRow(Grid: TNextGrid; Activity,Relevance: integer; Name,
  Curator, DateAdded, DateEdited, Web, Email, fType, ID, FIO, Rubr: string);
var
 s,Cur,Typ,Rub : string;
begin
 while pos('$',Curator) > 0 do begin
  ...
  if Length(Cur) > 0 then if Cur[1] = ',' then delete(Cur,1,2);
 end;
 while pos('$',fType) > 0 do begin
  ...
  if Length(Typ) > 0 then if Typ[1] = ',' then delete(Typ,1,2);
 end;
 while pos('$',Rubr) > 0 do begin
  ...
  if Length(Rub) > 0 then if Rub[1] = ',' then delete(Rub,1,2);
 end;
 Grid.AddRow;
 Grid.Cells[0,Grid.LastAddedRow] := ID;
 if Activity = -1 then
  Grid.Cells[1,Grid.LastAddedRow] := '1'
 else Grid.Cells[1,Grid.LastAddedRow] := '2';
 ...
 else Grid.Cells[11,Grid.LastAddedRow] := 'Нет';
end;
...
// t1, t2 : TDateTime;
begin
 if NOT LoadSGonRubrChange then exit;
// t1 := now;
 if TVRubrikator.Selected = nil then exit;
...
// if sPageControl1.ActivePageIndex <> 0 then sPageControl1.ActivePageIndex := 0;
 ...
 begin
  ...
  if IBQuery1.RecordCount = 0 then
   begin SGGeneral.ClearRows; IBQuery1.Close; IBDatabase1.Close; exit; end;
  SGGeneral.BeginUpdate;
  SGGeneral.ClearRows;
  ...
  begin
   SGAddRow(SGGeneral,IBQuery1.FieldByName('ACTIVITY').AsInteger,IBQuery1.FieldByName('RELEVANCE').AsInteger,
    IBQuery1.FieldValues['NAME'],IBQuery1.FieldValues['CURATOR'],IBQuery1.FieldValues['DATE_ADDED'],IBQuery1.FieldValues['DATE_EDITED'],
    IBQuery1.FieldValues['WEB'],IBQuery1.FieldValues['EMAIL'],IBQuery1.FieldValues['TYPE'],
...
begin
 with Sender as TNextGrid do begin
  ...
  for i := 0 to RowCount -1 do begin
   if NOT Odd(i) then C := $00EDE9EB {clMenuBar} else C := clWindow;
   for z := 0 to Columns.Count - 1 do Cell[z,i].Color := C;
  end;
  EndUpdate;
 end;
end;
...
begin
 ...
 begin
  ...
  begin
   if sPageControl1.Pages[i].Name = 'Tab'+ID
    then begin sPageControl1.Pages[i].Show; exit; end;
  end;
 end;
 if TNextGrid(Sender).Name = 'SGGeneral' then
 begin
  if SGGeneral.SelectedCount = 0 then exit;
...
procedure TFormMain.SGGeneralMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
 if Button = mbRight then begin
  if (x >= 0) and (x <= screen.Width) and (y >= 0) and (y <= 22) then
   PopupMenu_Columns.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y)
  else PopupMenu_Rubr.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
 end;
end;

procedure TFormMain.nCloseTabClick(Sender: TObject);
begin
 if sPageControl1.ActivePageIndex = 0 then exit;
...
begin
 ...
 begin
  while sPageControl1.PageCount > 1 do sPageControl1.Pages[1].Free;
  exit;
 end;
 curr := sPageControl1.ActivePage.Name;
 while sPageControl1.PageCount <> 2 do
 begin
   if NOT (sPageControl1.Pages[1].Name = curr) then sPageControl1.Pages[1].Free
   else if sPageControl1.PageCount > 2 then sPageControl1.Pages[2].Free;
 end;
...
begin
 if sPageControl1.PageCount = 1 then exit;
 while sPageControl1.PageCount > 1 do sPageControl1.Pages[1].Free;
end;
...
begin
 if FormDublicate.Visible then begin
 ...
 FormEditor.Show; FormEditor.EditName.SetFocus;
end;
...
begin
 ...
 begin
  ...
  FormEditor.SGRubr.Cells[0,FormEditor.SGRubr.LastAddedRow] :=
   TVRubrikator.Selected.Text;
  FormEditor.SGRubr.Cells[1,FormEditor.SGRubr.LastAddedRow] :=
   IntToStr(Integer(TVRubrikator.Selected.Data));
 end else begin
  FormEditor.Caption := 'Добавить фирму';
  FormEditor.BtnOK.Caption := 'Добавить';
  FormEditor.BtnOK.OnClick := FormEditor.AddRecord;
...
begin
 ...
 end else if SGGeneral.Focused then
 begin
  if SGGeneral.SelectedCount = 0 then exit;
  ID := SGGeneral.Cells[0,SGGeneral.SelectedRow];
 end else if TVRubrikator.Focused then
 begin
  if TVRubrikator.Selected = nil then exit;
  if TVRubrikator.Selected.Level = 0 then exit;
...
var
 ID : string;
begin
 if SGGeneral.Focused then begin
  if SGGeneral.SelectedCount = 0 then exit;
  ID := SGGeneral.Cells[0,SGGeneral.SelectedRow];
 end;
 if TVRubrikator.Focused then begin
  if TVRubrikator.Selected = nil then exit;
  if TVRubrikator.Selected.Level = 0 then exit;
  ID := IntToStr(Integer(TVRubrikator.Selected.Data));
 end;
...
begin
 if FormMailSender.Visible then begin
...
begin
 if (FormMailSender.Visible) AND (FormMailSender.Caption = 'Рассылка почты') then
  begin FormMailSender.SetFocus; exit; end;
 FormMailSender.Caption := 'Рассылка почты';
 FormMailSender.ClearEdits(True);
 FormMailSender.Show;
end;
...
begin
 ...
 begin
  ...
  begin
   ...
   FormMailSender.Show;
   FormMailSender.editTo.SetFocus;
   Q.Close; Q.Free; IBDatabase1.Close;
  end;
  if TVRubrikator.Selected.Level > 0 then
  begin
   ID := IntToStr(Integer(TVRubrikator.Selected.Data));
...
var
 ...
 Q : TIBQuery;
begin
 if FormMailSender.Caption <> 'Рассылка почты' then FormMailSender.ClearEdits(True);
 if SGGeneral.Focused then
 begin
  if SGGeneral.SelectedCount = 0 then exit;
  ID := SGGeneral.Cells[0,SGGeneral.SelectedRow];
...
var
 ...
 i,noactive : integer;
begin
 if NOT TVRubrikator.Focused then exit;
 if TVRubrikator.Selected = nil then exit;
 if TVRubrikator.Selected.Level <> 0 then exit;
 id := IntToStr(Integer(TVRubrikator.Selected.Data));
 noactive := 0; IBQuery1.Close;
 IBQuery1.SQL.Text := 'select ACTIVITY from BASE where RUBR like :RUBR';
 IBQuery1.Params[0].Value := '%#' + id +'$%';
 IBQuery1.Open; IBQuery1.FetchAll;
 for i := 1 to IBQuery1.RecordCount do begin
  if IBQuery1.FieldValues['ACTIVITY'] = '0' then inc(noactive);
  IBQuery1.Next;
 end;
 ...
 IBQuery1.Close; IBDatabase1.Close;
end;
...
var
 ...
 RE : TJvRichEdit;
begin
 if NOT FormReport.IsWordInstalled then begin
...
begin
 ...
 begin
  if sPageControl1.Pages[i].Name = 'Tab'+ID
   then begin sPageControl1.Pages[i].Free; Result := True; break; end;
 end;
end;
...
// ts.Visible := True;
 ...
 IBQuery1.Open;
 if IBQuery1.FieldValues['NAME'] <> null then begin
 ...
 begin
  Rubr := IBQuery1.FieldValues['CURATOR']; Cur := '';
  while pos('$',Rubr) > 0 do
  begin
  ...
  if Length(Cur) > 0 then if Cur[1] = ',' then delete(Cur,1,2);
  if Trim(Cur) <> '' then AddColoredLine(Cur,clWindowText,10,'Tahoma',[]);
 end;
 AddColoredLine('',clNone,10,'Times New Roman',[fsBold]);

 AddColoredLine('Рубрики:',clMaroon,10,'Tahoma',[fsBold]);
 if IBQuery1.FieldValues['RUBR'] <> null then Rubr := IBQuery1.FieldValues['RUBR'];
 while pos('$',Rubr) > 0 do
 begin
  tmp := copy(Rubr ,0 , pos('$',Rubr));
  delete(Rubr, 1 , length(tmp));
  ...
  Q.Database := IBDatabase1;
  Q.Transaction := IBTransaction1;
  Q.Close; Q.SQL.Text := 'select * from RUBRIKATOR where ID = :ID';
  Q.ParamByName('ID').AsString := tmp; Q.Open;
 ...
 if IBQuery1.FieldValues['NAPRAVLENIE'] <> null then Rubr := IBQuery1.FieldValues['NAPRAVLENIE'];
 while pos('$',Rubr) > 0 do
 begin
  tmp := copy(Rubr ,0 , pos('$',Rubr));
  delete(Rubr, 1 , length(tmp));
  ...
  Q.Database := IBDatabase1;
  Q.Transaction := IBTransaction1;
  Q.Close; Q.SQL.Text := 'select * from NAPRAVLENIE where ID = :ID';
  Q.ParamByName('ID').AsString := tmp; Q.Open;
  AddColoredLine(Q.FieldValues['NAME'],clWindowText,10,'Tahoma',[]);
  Q.Close; Q.Free;
 end;
 ...
 if Trim(IBQuery1.FieldValues['EMAIL']) <> '' then begin
 ...
 if Trim(IBQuery1.FieldValues['WEB']) <> '' then begin
...
// list2[4] = Street; list2[5] = Country; list2[6] = City;
  ...
  // такая же процедура в Report.GenerateReport и ReportSimple.GenerateReport и Editor.PrepareEdit
   ...
   ofType := list2[2]; if ofType[1] = '@' then delete(ofType,1,1);
   zip_str := list2[3];
   if Trim(ofType) <> '' then ofType := FormEditor.GetNameByID('OFFICETYPE',ofType) + ' - ';
   if Trim(zip_str) <> '' then zip_str := zip_str + ', ';
   if Trim(country_str) <> '' then country_str := FormEditor.GetNameByID('COUNTRY',country_str) + ', ';
   if Trim(city_str) <> '' then city_str := FormEditor.GetNameByID('GOROD',city_str) + ', ';
   adres := ofType + zip_str + country_str + city_str + list2[4];
   {officetype - zip, country, city, street}
   if Trim(adres) <> '' then AddColoredLine(adres,clWindowText,10,'Tahoma',[]);
   if Trim(tmp) <> '' then AddColoredLine(tmp,clNavy,10,'Tahoma',[]);
  end;
...
begin
 if Trim(editSearchFast.Text) = 'Поиск ...' then editSearchFast.Clear;
end;

procedure TFormMain.editSearchFastExit(Sender: TObject);
begin
 if Trim(editSearchFast.Text) = '' then editSearchFast.Text := 'Поиск ...';
end;

procedure TFormMain.editSearchFastKeyPress(Sender: TObject; var Key: Char);
begin
 if Key = #13 then begin Key := #0; SearchFast(editSearchFast); end;
end;
...
begin
 ...
 QuerySearch.Open; QuerySearch.FetchAll;
 if QuerySearch.RecordCount = 0 then
 begin
  QuerySearch.Close; QuerySearch.Free; IBDatabase1.Close;
  SGGeneral.ClearRows; exit;
 end;
 SGGeneral.BeginUpdate;
 SGGeneral.ClearRows;
 ...
 begin
  SGAddRow(SGGeneral,QuerySearch.FieldByName('ACTIVITY').AsInteger,QuerySearch.FieldByName('RELEVANCE').AsInteger,
   QuerySearch.FieldValues['NAME'],QuerySearch.FieldValues['CURATOR'],QuerySearch.FieldValues['DATE_ADDED'],QuerySearch.FieldValues['DATE_EDITED'],
   QuerySearch.FieldValues['WEB'],QuerySearch.FieldValues['EMAIL'],QuerySearch.FieldValues['TYPE'],
   QuerySearch.FieldValues['ID'],QuerySearch.FieldValues['FIO'],QuerySearch.FieldValues['RUBR']);
   QuerySearch.Next;
 end;
 SGGeneral.Resort;
 SGGeneral.EndUpdate;
 sPageControl1.ActivePageIndex := 0;
         ...
         MessageBox(Handle,PChar('Произошел сбой при поиске'+#13+E.Message),
          'Ошибка',MB_OK or MB_ICONERROR); end;
 end;
 QuerySearch.Close; QuerySearch.Free; IBDatabase1.Close;
end;
...
begin
 ...
 if FormReportSimple.Visible then
  begin FormReportSimple.SetFocus; exit; end;
...
begin
 ...
 if FormReport.Visible then
  begin FormReport.SetFocus; exit; end;
 FormReport.IsWordAviable(FormReport.cbLocGeneral,FormReport.cbLocWord);  
 FormReport.ClearEdits; FormReport.Show; FormReport.editSelect1.SetFocus;
end;
...
begin
 ...
 if (TVRubrikator.Focused) AND (TVRubrikator.Selected.Level = 0) then
 begin
  if FormReport.Visible then FormReport.Close;
  Report.Q_GEN_BREAK := False;
  FormReport.IsWordAviable(FormReport.cbLocGeneral,FormReport.cbLocWord);
  ...
  begin
   FormReport.editSelect2.ItemIndex := i; Break;
  end;
  FormReport.Show;
...
begin
 ...
 if (TVRubrikator.Focused) AND (TVRubrikator.Selected.Level = 0) then
 begin
  if FormReportSimple.Visible then FormReportSimple.Close;
  ReportSimple.Q_GEN_SIMPLE_BREAK := False;
  FormReport.IsWordAviable(FormReportSimple.cbLocGeneral,FormReportSimple.cbLocWord);
  FormReportSimple.ClearEdits;
  FormReportSimple.editFilter.ItemIndex := 0; {Рубрика}
  for i := 0 to sgRubr_tmp.RowCount - 1 do begin
  ...
  begin
   FormReportSimple.editFilterData.ItemIndex := i; Break;
  end;
  FormReportSimple.Show;
