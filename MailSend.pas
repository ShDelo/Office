...
type
  TFormMailSender = class(TForm)
    ...
    procedure LV_InsertFiles(nFile: string; ListView: TsListView;
    ...
    procedure GetEmailList(param: integer; Query: TIBQuery;
      ID: string; List: TsListBox; ClearList : Boolean);
    procedure GetFirmList(param: integer; Query: TIBQuery;
      ID: string; List: TsListBox);
...
begin
 ...
 memoLog.Left := 6; memoLog.Top := 220;
 memoLog.Width := 532; memoLog.Height := 225;
 memoLog.Visible := False; memoLog.Clear;
 ...
 REFirmInfo.Visible := False; REFirmInfo.Clear;
 REFirmInfo.WordWrap := False;
end;

procedure TFormMailSender.FormClose(Sender: TObject;
...
begin
 ...
 else if editFrom.Items.Count > 0 then editFrom.ItemIndex := 0;
 IniMain.Free;
end;

procedure TFormMailSender.ClearEdits(ClearList: Boolean);
begin
 if FormMailSender.Caption = 'Рассылка почты' then begin
  editTo.Enabled := True; btnEmailAdd.Enabled := True;
  btnEmailDelete.Enabled := True; btnSend.OnClick := SendEmail;
 end else begin
  editTo.Enabled := False; btnEmailAdd.Enabled := False;
  btnEmailDelete.Enabled := False; btnSend.OnClick := SendRegInfoCheck;
 end;
 sPageControl.ActivePageIndex := 0;
 editSubj.Clear; editAttach.Clear;
 editMessage.Clear; memoLog.Clear; editTo.Clear;
 if ClearList then editEmailList.Clear;
...
var
 i : integer;
begin
 editProfile.ItemIndex := -1; editProfile.Clear;
 ...
 if FormMain.IBQuery1.RecordCount > 0 then
  ...
  begin
   ...
   editFrom.Items.Add(FormMain.IBQuery1.FieldValues['DOMEN']+
    ' ('+FormMain.IBQuery1.FieldValues['NAME']+')');
   FormMain.IBQuery1.Next;
  end;
 if editProfile.Items.Count > 0 then editProfile.ItemIndex := 0;
 if editProfile.Items.Count > 0 then editProfileChange(editProfile);
 FormMain.IBQuery1.Close;
end;

procedure TFormMailSender.LV_InsertFiles(nFile: string;
  ListView: TsListView; ImageList: TsAlphaImageList);
 ...
 begin
  if bytes = 0 then begin result := '0 байт'; exit; end;
  if bytes > GB then result := FormatFloat('#.## ГБ', bytes / GB)
  else if bytes > MB then result := FormatFloat('#.## МБ', bytes / MB)
  else if bytes > KB then result := FormatFloat('#.## КБ', bytes / KB)
  else result := FormatFloat('#.## байт', bytes) ;
 end;
...
begin
 SHGetFileInfo(PChar(nFile), 0, FileInfo,
...
procedure TFormMailSender.IdSMTPStatus(ASender: TObject;
  const AStatus: TIdStatus; const AStatusText: String);
begin
 memoLog.Lines.Insert(0, '['+TimeToStr(Now)+'] Status: ' + AStatusText);
end;

procedure TFormMailSender.IdSMTPFailedRecipient(Sender: TObject;
...
procedure TFormMailSender.IdMessageInitializeISO(
...
begin
 if editProfile.ItemIndex < 0 then exit;
 FormMain.IBQuery1.Close;
...
var
 ...
 pass_encoded : string;
begin
 if InputQuery('Профиль','Название:',profile) then if Trim(profile) <> '' then
 begin
  ...
  if NOT InputQuery(profile,'Сервер отправки почти (SMTP server):',host) then exit;
  if Trim(host) = '' then exit;
  if NOT InputQuery(profile,'Порт:',port) then exit;
  if Trim(port) = '' then exit;
  InputQuery(profile,'Имя отправителя (не обязательно):',uname);
  InputQuery(profile,'Обратный адрес (не обязательно):',replyto);
  if NOT InputQuery(profile,'Домен (адрес электронной почты):',domen) then exit;
  if Trim(domen) = '' then exit;
  if NOT InputQuery(profile,'Логин:',login) then exit;
  if Trim(login) = '' then exit;
  if NOT InputQuery(profile,'Пароль:',pass) then exit;
  if Trim(pass) = '' then exit;

  FormMain.IBQuery1.Close;
  FormMain.IBQuery1.SQL.Text := 'insert into ACCOUNTS (NAME,HOST,PORT,UNAME,REPLYTO,DOMEN,LOGIN,PASS,SIGNATURE,SSL_METHOD,TLS_METHOD) values'+
  ...
  pass := Trim(pass); pass_encoded := EncodeString(pass);
  ...
  else FormMain.IBQuery1.ParamByName('TLS_METHOD').AsInteger := 0;
  ...
  editHost.Text := host; editPort.Text := port;
  editReplyTo.Text := replyto; editUName.Text := uname;
  editDomen.Text := domen; editLogin.Text := login;
  editPassword.Text := pass; editSignature.Clear;
...
begin
 ...
 pass := Trim(editPassword.Text); pass_encoded := EncodeString(pass);
 FormMain.IBQuery1.ParamByName('PASS').AsString := pass_encoded;
...
begin
 if editProfile.ItemIndex < 0 then exit;
 if MessageBox(handle,'Профиль будет удален. Продолжить?','Подтверждение',
 ...
 begin
  editProfile.ItemIndex := 0; editProfileChange(editProfile);
 end else
 begin
  editProfile.Clear; editHost.Clear; editPort.Clear; editUName.Clear;
  editReplyTo.Clear; editDomen.Clear; editLogin.Clear; editPassword.Clear;
  editSignature.Clear; editSSLMethod.ItemIndex := -1; editTLSMethod.ItemIndex := -1;
 end;
end;
...
begin
 if editAttach.SelCount = 0 then exit;
 editAttach.Selected.Delete;
 if editAttach.Items.Count > 0 then editAttach.Items[0].Selected := True;
end;

procedure TFormMailSender.editToKeyPress(Sender: TObject; var Key: Char);
begin
 if Key = #13 then begin Key := #0; btnEmailAddClick(btnEmailAdd); end;
end;

procedure TFormMailSender.editEmailListKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
 if FormMailSender.Caption = 'Рассылка почты' then
 if Key = 46 then begin Key := 0; btnEmailDeleteClick(btnEmailDelete); end;
end;
...
var
 i : integer;
begin
 if Trim(editTo.Text) = '' then exit;
 if NOT isValidEmail(editTo.Text) then begin
 ...
 for i := 0 to editEmailList.Items.Count - 1 do
  if editEmailList.Items[i] = Trim(editTo.Text) then exit;
...
var
 i : integer;
begin
 if editEmailList.ItemIndex = -1 then exit;
...
begin
 ...
 if (AnsiLowerCase(wwwPart) <> 'www') or ((Length(domenPart) < 5)) then exit;
 i:= Pos('.', domenPart);
 if (i = 0) or (i > (Length(domenPart) - 2)) then exit;
 Result:= CheckAllowed(wwwPart) and CheckAllowed(domenPart);
end;

procedure TFormMailSender.GetEmailList(param: integer; Query: TIBQuery;
  ID: string; List: TsListBox; ClearList : Boolean);
var
 i : integer;
 email,tmp : string;
...
begin
 ...
 begin
  ...
  begin
   email := '';
   if Query.FieldValues['EMAIL'] <> null then email := Query.FieldValues['EMAIL'];
   ...
   begin
    ...
    begin
     ...
     if NOT IsValidEmail(tmp) then inList := True;
     if List.Items.IndexOf(tmp) > -1 then inList := True;
     if NOT inList then List.Items.Add(tmp);
    end;
   end;
   Query.Next;
  end;
 ...
 begin
  ...
  email := '';
  if Q.FieldValues['EMAIL'] <> null then email := Q.FieldValues['EMAIL'];
  if Length(email) > 0 then
  begin
   if email[Length(email)] <> ',' then email := email + ',';
   while pos(',',email) > 0 do
 ...
 if FormMain.IBDatabase1.Connected then FormMain.IBDatabase1.Close;
end;

procedure TFormMailSender.GetFirmList(param: integer; Query: TIBQuery;
  ID: string; List: TsListBox);
var
 i : integer;
 Q : TIBQuery;
begin
 if param = 0 then // работаем с Query
 begin
  List.Clear; editEmailList_tmp.Clear;
  if Query.RecordCount = 0 then exit;
  Query.Last; Query.First;
  for i := 0 to Query.RecordCount - 1 do
  begin
...
begin
 if editFrom.ItemIndex < 0 then begin
  MessageBox(handle,'Необходимо указать отправителя','Информация',MB_OK or MB_ICONINFORMATION);
  exit;
 end;
 if Trim(editTo.Text) <> '' then btnEmailAddClick(btnEmailAdd);
 if editEmailList.Items.Count = 0 then begin
...
{ if Trim(editMessage.Text) = '' then
 ...
 if FormMain.IBQuery1.RecordCount = 0 then
 begin
  btnLog.Caption := 'Скрыть лог'; memoLog.Visible := True;
 ...
 IdMessage.Date := now;
 if nPriorityHigh.Checked then IdMessage.Priority := mpHigh;
 if nPriorityMed.Checked then IdMessage.Priority := mpNormal;
 if nPriorityLow.Checked then IdMessage.Priority := mpLow;
{ for i := 0 to editEmailList.Items.Count - 1 do
 ...
 begin
  tmp := editAttach.Items[i].Caption; attachCapt := '';
  for n := length(tmp) downto 0 do
   if tmp[n] = '(' then attachCapt := copy(tmp,0,n-2);
  if FileExists(editAttach.Items[i].SubItems[0] + attachCapt) then begin
 ...
 IdSMTP.Host := host;
 try IdSMTP.Port := StrToInt(port); // обычно при использование ssl 495, 587 или стандартный 25
 except on E:Exception do begin
 ...
 case SSL_Method of
  0 : IdSSLIOHandlerSocketOpenSSL.SSLOptions.Method := sslvTLSv1;
  1 : IdSSLIOHandlerSocketOpenSSL.SSLOptions.Method := sslvSSLv2;
 end;
...
// idSMTP.ConnectTimeout := 10;
 ...
 try
  ...
  Gauge.Progress := 0; Gauge.MaxValue := editEmailList.Items.Count;
  lblGauge.Caption := ''; lblGauge.Visible := True; Gauge.Visible := True;
  for i := 0 to editEmailList.Items.Count - 1 do
  begin
   if MailSend_Break then Break;
  ...
  if Gauge.Visible then Gauge.Visible := False;
  if lblGauge.Visible then lblGauge.Visible := False;
  btnCancel.Caption := 'Закрыть';
 end;
 btnSend.Enabled := True;
 if NOT isError then begin
  btnLog.Caption := 'Показать лог'; memoLog.Visible := False;
...
var
 ...
 var
  ...
  begin
   ...
   REFirmInfo.SelAttributes.Size := AFontSize; REFirmInfo.SelAttributes.Name := AFontName;
   REFirmInfo.SelAttributes.Style := AFontStyle; REFirmInfo.Lines.Add(AText);
  end;
  ...
  begin
   ...
   begin        
    ...
    if (pos('@',tmp) = 0) or (pos('.',tmp) = 0) {можно делать IsValidEMail}
     or (pos(',',tmp) > 0) then inList := True;
    if EmailsList.IndexOf(tmp) <> -1 then InList := True;
    if NOT inList then EmailsList.Add(tmp);
   end;
  end;
  if EmailsList.Count > 0 then begin // если emails найдены получаем инфо фирмы
   if Q.FieldValues['NAME'] <> null then begin
   ...
   if Q.FieldValues['RUBR'] <> null then Rubr := Q.FieldValues['RUBR'];
   while pos('$',Rubr) > 0 do
   begin
    ...
    Q1.Close; Q1.SQL.Text := 'select * from RUBRIKATOR where ID = :ID';
    Q1.ParamByName('ID').AsString := tmp; Q1.Open;
   ...
   if Q.FieldValues['NAPRAVLENIE'] <> null then Rubr := Q.FieldValues['NAPRAVLENIE'];
   while pos('$',Rubr) > 0 do
   begin
    tmp := copy(Rubr ,0 , pos('$',Rubr));
    delete(Rubr, 1 , length(tmp));
    ...
    Q1.Database := FormMain.IBDatabase1;
    Q1.Transaction := FormMain.IBTransaction1;
    Q1.Close; Q1.SQL.Text := 'select * from NAPRAVLENIE where ID = :ID';
    Q1.ParamByName('ID').AsString := tmp; Q1.Open;
    AddColoredLine(Q1.FieldValues['NAME'],clWindowText,10,'Tahoma',[]);
    Q1.Close; Q1.Free;
   end;
   ...
   if Trim(Q.FieldValues['EMAIL']) <> '' then begin
   ...
   if Trim(Q.FieldValues['WEB']) <> '' then begin
...
begin
 ...
 if editEmailList_tmp.Items.Count = 0 then begin
 ...
 try
  ...
  MailSend_Break := False;
  btnCancel.Caption := 'Отмена';
  Gauge.Progress := 0; Gauge.MaxValue := editEmailList_tmp.Items.Count;
  lblGauge.Caption := ''; lblGauge.Visible := True; Gauge.Visible := True;
  for z := 0 to editEmailList_tmp.Items.Count - 1 do begin
   if MailSend_Break then Break;
   lblGauge.Caption := IntToStr(z+1)+' из '+IntToStr(editEmailList_tmp.Items.Count);
   Gauge.Progress := z+1;   
   Application.ProcessMessages;
   if NOT (GetFirmData(editEmailList_tmp.Items[z])) then Continue;
   {формируем тело сообщения}
   IdMessage.Clear;
   IdMessage.MessageParts.Clear;
   IdMessage.Priority := mpNormal;
