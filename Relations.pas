...
begin
 ...
 if FormMain.IBQuery1.FieldValues['NAPRAVLENIE'] <> null then begin
 ...
 FormMain.IBQuery1.Close;
 while Pos('$',RUBRstr) > 0 do begin
  str1 := copy(RUBRstr ,0 , pos('$',RUBRstr));
  delete(RUBRstr, 1 , length(str1));
  delete(str1,1,1); delete(str1,length(str1),1);
  editRubrRelations.AddItem(FormEditor.GetNameByID('RUBRIKATOR',str1),Pointer(StrToInt(str1)));
 end;
 if editRubrRelations.Items.Count > 0 then editRubrRelations.ItemIndex := 0;
 while Pos('$',RELATIONSstr) > 0 do begin
...
begin
 if SGNaprAviable.SelectedCount = 0 then exit;
...
begin
 if SGNaprSelected.SelectedCount = 0 then exit;
...
var
 i : integer;
begin
 if SGNaprAviable.RowCount = 0 then exit;
 SGNaprAviable.BeginUpdate; SGNaprSelected.BeginUpdate;
 for i := 0 to SGNaprAviable.RowCount - 1 do begin
  SGNaprSelected.AddRow;
 ...
 SGNaprAviable.ClearRows;
 SGNaprAviable.EndUpdate; SGNaprSelected.EndUpdate;
 BuildRelationsString(IntToStr(Integer(editRubrRelations.Items.Objects[editRubrRelations.ItemIndex])));
end;
...
var
 i : integer;
begin
 if SGNaprSelected.RowCount = 0 then exit;
 SGNaprAviable.BeginUpdate; SGNaprSelected.BeginUpdate;
 for i := 0 to SGNaprSelected.RowCount - 1 do begin
  SGNaprAviable.AddRow;
...
begin
 Result := '';
 if listRubrRelations.Count = 0 then exit;
 for i := 0 to listRubrRelations.Count - 1 do begin
  if Pos('#'+RubrID+'=',listRubrRelations[i]) > 0 then begin
   finalStr := '#'+RubrID+'=';
   for x := 0 to SGNaprSelected.RowCount - 1 do begin
...
begin
 if editRubrRelations.ItemIndex = -1 then exit;
 SGNaprAviable.BeginUpdate; SGNaprSelected.BeginUpdate;
 ...
 for i := 0 to listRubrRelations.Count - 1 do begin
  if Pos('#'+RubrID+'=',listRubrRelations[i]) > 0 then begin
   str1 := listRubrRelations[i]; Break;
